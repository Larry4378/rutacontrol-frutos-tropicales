...
function RouteMap({data,onUpdate}) {
  ...
  return <section className="route-section route-navigation"><div className="section-head route-section-title"><div><p className="eyebrow">SEGUIMIENTO</p><h2>Trayecto en tiempo real</h2><p>{active ? `Movilidad ${vehicleName(data,active.vehicleId)} en ruta.` : 'Inicia una salida para visualizar el recorrido.'}</p></div>{tracking&&<span className="tracking-badge">● GPS en vivo</span>}</div><div className="route-map-shell"><div ref={mapNode} className="route-map"/>{active&&<><div className="route-map-status"><span className="route-live-dot"/><div><b>{vehicleName(data,active.vehicleId)}</b><small>{tracking?'Ubicación actualizándose':'Ubicación detenida'}</small></div></div><button type="button" className="map-recenter-button" onClick={focusVehicle} title="Centrar mi vehículo" aria-label="Centrar mi vehículo">⌖</button></>}</div><p className="route-note">{message} {points>1&&' La línea azul muestra el trayecto registrado.'}</p></section>;
}
