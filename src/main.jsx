...
function App() {
  ...
  useEffect(() => {
    const changeSession = nextSession => {
      ...
      // Supabase renueva el token periódicamente. Si continúa siendo la misma
      // persona, no vaciamos su perfil ni su vehículo durante la renovación.
  ...
  useEffect(() => {
    ...
    supabase.from('vehicles').select('id, plate, brand, model, vehicle_type, current_km, ownership, status').order('plate')
      .then(({ data: vehicles, error: loadError }) => {
        ...
        if (loadError) {
          setVehiclesReady(false);
          return setError(`No se pudieron cargar los vehículos: ${loadError.message}`);
        }
  ...
  const loadUsers = async () => {
    ...
    setProfileReady(false);
    const { data: own, error: profileError } = await supabase.from('user_profiles').select('id,full_name,role,is_active,permissions').eq('id', session.user.id).maybeSingle();
    if (profileError || !own) {
      setProfile(null);
      setProfileReady(false);
      setError(profileError ? `No se pudo cargar tu asignación: ${profileError.message}` : 'Tu usuario no tiene un perfil activo asignado.');
      return;
    }
    setProfile(own || null);
    let adminDrivers = [];
...
function RouteMap({ data, profile, driverPreview, onUpdate, gpsPresentation = false }) {
  ...
  useEffect(() => {
    ...
    const tiles = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OpenStreetMap contributors', maxNativeZoom: 19, maxZoom: 20 });
    tiles.on('load', refreshMapSize).addTo(map.current);
    L.control.zoom({ position: 'bottomright' }).addTo(map.current);
    L.control.attribution({ position: 'bottomleft', prefix: '© OpenStreetMap' }).addTo(map.current);
    const resizeObserver = typeof ResizeObserver === 'function' ? new ResizeObserver(refreshMapSize) : null;
    resizeObserver?.observe(mapNode.current);
    window.addEventListener('resize', refreshMapSize);
    window.addEventListener('orientationchange', refreshMapSize);
    document.addEventListener('visibilitychange', refreshMapSize);
    const refreshFrame = requestAnimationFrame(refreshMapSize);
    const refreshTimers = [80, 250, 600].map(delay => window.setTimeout(refreshMapSize, delay));
    return () => {
      map.current?.off('dragstart', markManualView);
      map.current?.off('zoomstart', markManualView);
      tiles.off('load', refreshMapSize);
      resizeObserver?.disconnect();
      window.removeEventListener('resize', refreshMapSize);
      window.removeEventListener('orientationchange', refreshMapSize);
      document.removeEventListener('visibilitychange', refreshMapSize);
...
function Permissions({driver,vehicles,onSave}) {
  const defaults={departure:true,arrival:true,trips:true,fuel:false,kpi:false};
  const [permissions,setPermissions]=useState({...defaults,...(driver.permissions||{})});
  const [editing,setEditing]=useState(false);
  const options=[['departure','Salida'],['arrival','Llegada'],['trips','Recorridos propios'],['fuel','Combustible'],['kpi','Rendimiento Km/Gl · KPI']];
...
function PhotoSource({ onChange, onStored, accept = 'image/*', withCamera = true, showPreview = false }) {
  ...
  const select = async event => {
    ...
    if (accepted === false) {
      setPreviewUrl('');
      setFileName('No se pudo adjuntar la foto. Inténtalo nuevamente.');
      input.value = '';
      return;
    }
...
function DepartureGpsRequired({ data, drivers = [], driverName = '', driverId = '', assignedVehicleId = '', assignedVehicleLabel = '', onClose, onSave }) {
  ...
  const ocr = async file => {
    ...
    change('startPhoto', '');
    setStatus('Foto adjuntada. Puedes revisarla abajo mientras se guarda; escribe manualmente el kilometraje mostrado.');
    return true;
  };
...
function ArrivalSimple({ data, driverName = '', driverId = '', onClose, onSave }) {
  ...
  const storeArrivalPhoto = ({ path }) => {
    ...
    setPhotoSelected(true);
    setStatus('Foto guardada correctamente. Revisa la vista previa y escribe manualmente el kilometraje final.');
  };
  const submit = async event => {
    event.preventDefault();
    if (saving) return;
  ...
  return (
    <dialog open className="quick-departure-modal">
      <form className="arrival-form" onSubmit={submit} noValidate>
        ...
        {active.length === 0 ? <p className="empty-message">No hay una salida pendiente.</p> : <>
          <div className="form-grid">
            ...
            <div className="field full">
              ...
              <PhotoSource onChange={event => odometer(event.target.files?.[0])} onStored={storeArrivalPhoto} showPreview />
            </div>
            <div className="field full">
