...
function DepartureGpsRequired({ data, driverName = '', assignedVehicleId = '', assignedVehicleLabel = '', onClose, onSave }) {
  ...
  const pendingTrip = data.trips.find(trip => !trip.endKm && form.driver && String(trip.driver || '').trim().toLowerCase() === String(form.driver).trim().toLowerCase());
  const change = (key, value) => setForm(current => ({ ...current, [key]: value }));
...
function ArrivalSimple({ data, driverName = '', onClose, onSave }) {
  const active = data.trips.filter(trip => !trip.endKm && (!driverName || String(trip.driver || '').trim().toLowerCase() === String(driverName).trim().toLowerCase()));
