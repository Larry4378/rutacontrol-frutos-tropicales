...
function MaintenanceModal({record={},data,assignedVehicleId='',onClose,onSave}) {
  ...
  return <dialog open className="maintenance-modal"><form onSubmit={submit}>
    ...
    <div className="form-grid maintenance-grid-form">
      ...
      <div className="field"><label>Próxima fecha</label><input type="date" value={form.nextDate||''} onChange={e=>change('nextDate',e.target.value)}/></div>
      <div className="field"><label>Próximo kilometraje</label><input type="number" min="0" step="0.1" value={form.nextKm||''} onChange={e=>change('nextKm',e.target.value)}/></div>
    </div>
    <div className="form-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary">Guardar mantenimiento</button></div>
  </form></dialog>;
}
