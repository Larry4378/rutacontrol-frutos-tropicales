...
Deno.serve(async (req) => {
  ...
  const { qrToken, pin } = await req.json()
  if (!qrToken || !/^\d{6}$/.test(String(pin || ''))) return reply({ error: 'Escanea tu QR e ingresa tu PIN de 6 números.' }, 400)
  const url = Deno.env.get('SUPABASE_URL')!
  const service = createClient(url, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!)
  const { data: profile } = await service.from('user_profiles').select('id,is_active').eq('qr_token', qrToken).eq('role', 'driver').maybeSingle()
